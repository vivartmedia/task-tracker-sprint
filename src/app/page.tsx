...
export default function Home() {
  ...
  return (
   ...
   <div className="grid grid-flow-row justify-center mt-20">
      <div className="bg bg-[#3EBE9F] min-w-96 p-16 rounded-sm">
      ...
      <div>
        ...
        <TextInput className="w-full border-none rounded-md" id="password1" type="password" required onChange={(e) => setPassword(e.target.value)}/>
      </div>
        {switchBool && ( 
            <div>
              <div className="mb-1 block">
              ...
              <TextInput className="w-full border-none rounded-md" id="password2" type="password" required onChange={(e) => setConfirmPassword(e.target.value)} />
            </div>
          )}
      ...
      <Button className="hammer" onClick={handleSwitch} color="light">{switchBool ?  <span>Already Have an Account? <u>Login</u></span> : <span>Dont have an account? <u>Create one now!</u></span> }</Button>
      </div>
    </form>
      </div>
   </div>
